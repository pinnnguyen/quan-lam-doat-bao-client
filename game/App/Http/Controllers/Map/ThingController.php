            ...
            foreach ($map_things_bodies as $key => $map_things_body) {
                if ($map_things_body['expire'] > time()) {
                    if ($map_things_body['expire'] - time() > 180) {
                        $things[] = ['name' => 'Một khối' . $map_things_body['name'] . '的尸体', 'viewUrl' => 'Map/Thing/viewBody/' . $key];
                    } elseif ($map_things_body['expire'] - time() > 120) {
                        $things[] = ['name' => 'Một khối hư thối thi thể', 'viewUrl' => 'Map/Thing/viewBody/' . $key];
                    } elseif ($map_things_body['expire'] - time() > 60) {
                        $things[] = ['name' => 'Một khối khô cạn thi thể', 'viewUrl' => 'Map/Thing/viewBody/' . $key];
                    } else {
                        $things[] = ['name' => 'Một đống hài cốt', 'viewUrl' => 'Map/Thing/viewBody/' . $key];
                    }
                }
            }
        }
        ...
        if ($map_things['xinfas']) {
            ...
            foreach ($map_things_xinfas as $key => $map_things_xinfa) {
                if ($map_things_xinfa['expire'] > time() and !empty($map_things_xinfa['xinfa_id'])) {
                    ...
                    $things[] = ['name' => 'Một quyển' . $xinfa->name, 'viewUrl' => 'Map/Thing/viewXinfa/' . $key];
                }
            }
        }
        ...
        if ($map_money) {
            ...
            if ($money['number'] > 0 or ($money['is_no_expire'] or $money['expire'] > time())) {
                ...
                return $connection->send(\cache_response($request, \view('Base/message.twig', [
                    'request' => $request,
                    'message' => 'Nơi này cái gì cũng không có.',
                ])));
            }
        } else {
            return $connection->send(\cache_response($request, \view('Base/message.twig', [
        ...
        if ($map_money) {
            ...
            if ($money['number'] > 0 or ($money['is_no_expire'] or $money['expire'] > time())) {
                if ($money['expire'] - time() > 240 and $money['protect_role_id'] != 0 and $money['protect_role_id'] != $request->roleId) {
                    $message = 'Này không phải ngươi đồ vật';
                } else {
                    $role_attrs = Helpers::getRoleAttrsByRoleId($request->roleId);
                    if ($role_attrs->reviveTimestamp > time()) {
                        return $connection->send(\cache_response($request, \view('Base/message.twig', [
                            'request' => $request,
                            'message' => 'Ngươi vẫn là một cái quỷ hồn, vô pháp nhặt lên trên mặt đất vật phẩm.',
                        ])));
                    }
                    if ($role_attrs->weight + $money['number'] >= 100000000) {
                        return $connection->send(\cache_response($request, \view('Base/message.twig', [
                            'request' => $request,
                            'message' => 'Ngươi ba lô đã đầy.',
                        ])));
                    }
                    cache()->hDel('map_things_' . $request->roleRow->map_id, 'money');
                    $message = 'Ngươi nhặt lên' . Helpers::getHansMoney($money['number']) . '。';
                    $sql = <<<SQL
...
SQL;
                    ...
                    loglog(LOG_PICK_UP_THINGS, '拾取金钱', [
                        ...
                        '地图' => Helpers::getMapRowByMapId($request->roleRow->map_id)->name,
                        '数量' => $money['number'],
                    ]);
    ...
    public function pickUpThing(TcpConnection $connection, Request $request, string $key)
    {
        ...
        $thing = $things[$key];
        if ($thing['expire'] - time() > 240 && $thing['protect_role_id'] != 0 && $thing['protect_role_id'] != $request->roleId) {
        ...
        if ($thing['row']->kind === '装备' and $thing['protect_role_id'] != $request->roleId) {
            ...
            if (is_array($maps) and count($maps) >= 20 and !in_array($request->roleRow->map_id, $maps)) {
                return $connection->send(\cache_response($request, \view('Base/message.twig', [
                    'request' => $request,
                    'message' => 'Ngươi đã ở vượt qua 20 cái bản đồ nhặt trang bị, thỉnh hợp lý nhặt.',
                ])));
            }
            /**
             * 设定最后一次拾取时间 地图
...
SQL;
        ...
        $message = 'Ngươi nhặt lên一' . $thing['row']->unit . $thing['row']->name . '。';
        FlushRoleAttrs::fromRoleThingByRoleId($request->roleId);
...
SQL;
                            ...
                            $message = 'Ngươi nhặt lênMột quyển' . $xinfa['row']->name . '。';
                        }
                    }
            ...
            if (empty($bodies[$key])) {
                ...
                if ($bodies[$key]['expire'] < time()) {
                    ...
                    if ($body['expire'] - time() > 180) {
                        $body['name'] = 'Một khối' . $body['name'] . ' Thi thể';
                    } elseif ($body['expire'] - time() > 120) {
                        $body['name'] = 'Một khối hư thối thi thể';
                    } elseif ($body['expire'] - time() > 60) {
                        $body['name'] = 'Một khối khô cạn thi thể';
                    } else {
                        $body['name'] = 'Một đống hài cốt';
                    }
...
SQL;
                    ...
                    return $connection->send(\cache_response($request, \view('Map/Thing/pickUpBody.twig', [
                        'request' => $request,
                        'message' => 'Ngươi nhặt lên ' . $body['name'] . '。',
                    ])));
                }
            }
        } else {
